...
export const VerifyUserForgot = ({ dataForgot }) => {
    ...
    const saveAccount = async (data) => { // Thêm async
        try {
            const res = await axios.post('http://127.0.0.1:8000/api/authentication/forgot-password/save', {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://127.0.0.1:8000/api/authentication/forgot-password/status', {
                email: dataForgot?.email || '', // Kiểm tra email trước khi gửi
                otp: otp,
            },
                {
                    headers: {
            ...
            if (response.status === 200) {
                ...
                // Lưu tài khoản sau khi OTP xác thực thành công
                const isAccountSaved = await saveAccount(dataForgot);
